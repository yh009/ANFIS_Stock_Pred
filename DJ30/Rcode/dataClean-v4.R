...
# Update: 1. remove original P.B.ratio and Free.cash.flow.per.share as some stocks has missing entries
#         2. capped between 1995-01-01 and 2018-01-01
...
for (i in 1:length(file.names)){
  ...
  selectedData = rawdata[, -which(names(rawdata) %in% 
                                    c("Shares", "Split.factor", "Non.controlling.interest","Preferred.equity","Goodwill...intangibles","Long.term.debt",
                                      ...
                                      "P.B.ratio","Free.cash.flow.per.share"))]
  ...
  selectedData[,2:19] <- as.data.frame(lapply(selectedData[,2:19], function(x) {
    x <- as.character(x)
    x[x == "None"] <- NA
    as.numeric(x)
  }))
  ...
  selectedData$BPS = round(selectedData$Book.value / selectedData$Shares.split.adjusted, digits = 4)
  
  selectedData$PE = round(selectedData$Price / selectedData$BPS, digits = 4)
  ...
  date = as.yearmon(as.character(selectedData$Quarter.end))
  ...
  # Create xts object
  myXts = xts(selectedData[,2:22], date)
  # colnames(myXts)
  ...
  myXts$BPS.Q = round(diff(myXts$BPS, lag = 1)/lag(myXts$BPS)*100,4)
  ...
  myDataFinal = myData[myData$fakeDate>=as.Date("1995-01-01") & myData$fakeDate<=as.Date("2018-01-01"),]
  
  myDataFinal = myDataFinal[, -which(names(myDataFinal) %in% c("fakeDate"))]
